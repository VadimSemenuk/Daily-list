export default {
    // settings page
    "view": "Адлюстраванне",
    "interface": "Інтэрфейс",
    "add-pass": "Дадаць пароль",
    "remove-pass": "Выдаліць пароль", 
    "issues": "Ліквідацыя няспраўнасцяў",
    "default-notification": "Аўтаматычна дадаваць апавяшчэннеы",
    "backup": "Рэзервовае капіраванне",
    "set-setting-error": "Не атрымалася абнавіць наладу. Перазапусціце праграму і попробуйие зноў",
    "restore-backup-confirmation": "Аднавіць рэзервовую копію?",

    // view settings page
    "sort": "Сартаванне",
    "time-sort": "Па усталяваным часе",
    "time-add-sort": "Па часе дадання",
    "custom-sort": "Ўласная сартаванне",
    "view-direction": "Парадак адлюстравання",  
    "view-direction-asc": "У прамым парадку",
    "view-direction-desc": "У адваротным парадку",
    "fin-sort": "Перамяшчаць завершаныя ўніз",
    "notes-show-interval": "Адлюстроўваць за перыяд",
    "notes-show-interval-week": "Тыдзень",
    "notes-show-interval-day": "Дзень",
    "time-sort-notification": "Пры сартаванні па згаданага часу не даступная сартаванне перетягиваением",

    // interface page
    "theme": "Тэма",
    "random-theme": "Выпадковая тэма",
    "font-size": "Памер шрыфта нататкі",
    "show-mini-calendar": "Паказваць міні-каляндар",
    "language": "Мова",
    "ru": "Руссский",
    "en": "English",
    "be": "Беларускі",
    "notes-count": "Лічыльнік нататак на календары",
    "notes-count-not-show": "Не адлюстроўваць",
    "notes-count-only-finished": "Адлюстроўваць толькі завершаныя",
    "notes-count-all": "Адлюстроўваць усе",
    "minimize-notes": "Згортваць доўгія нататкі",
    "move-control-panels-down": "Перанесці ўніз элементы кіравання",
    "quick-add-panel": "Панэль хуткага дадання",

    // password page
    "password": "Пароль",
    "invalid-password": "Няправільны пароль",
    "pass-in": "Увядзіце пароль",
    "not-equal-passwords": "Паролі ў палях не супадаюць",
    "no-space-password": "У паролі нельга выкарыстоўваць прабел",
    "min-symbols-password": "Увядзіце мінімум 4 сімвала",
    "new-password": "Увядзіце новы пароль",
    "repeat-password": "Увядзіце пароль паўторна",
    "password-reset-email": "Email для скіду пароля",
    "reset-password-email-no-user": "Ўвайдзіце што б прывязаць email для сбросса пароля",
    "reset-password": "Скінуць пароль",
    "password-has-been-reset": "Новы пароль адпраўлены на адрас {email}. Праверце папку \"Спам\" калі ліст не знойдзена",
    "reset-password-notification": "Пакажыце email адрас для скіду пароля. Для гэтага ўвайдзіце ў аккаутн ў раздзелах 'Рэзервовае капіраванне' ці 'Дадаць пароль'",
    "password-reset-email-subject": "Скід пароля",
    "password-reset-email-content": "Новы пароль: ",

    //troubleshooting page
    "help": "Дапамога",
    "previous-version": "Папярэдняя версія", 
    "no-notification": "Не прыходзяць апавяшчэння",
    "no-notification-description": `Перайдзіце ў налады прыкладання ў сістэме: Налады -> Прыкладанні -> Штодзённік.
    Пераканайцеся што ў дадатку ёсць дазволу на апавяшчэння і працу ў фоне`,

    // about page
    "share-app": "Падзяліцца праграмай",
    "resources": "Выкарыстаныя рэсурсы",
    "graphics": "Графіка",

    // backup page
    "copy-auto-created-google-drive": "Аўтаматычна створана на",
    "copy-auto-created-local": "Аўтаматычна створана лакальна",
    "google-sign-in": "Увайсці з дапамогай Google",
    "sign-out": "Выйсці",
    "create-backup": "Загрузіць",
    "restore-backup": "Аднавіць",
    "copy": "Копія",
    "available-copies": "Даступныя рэзервовыя копіі",
    "available-local-copies": "Даступныя лакальныя рэзервовыя копіі",
    "auth-error": "Адбылася памылка. Паўтарыце пазней",
    "restore-local-backup": "Аднавіць лькальную копію",
    "how-it-works-btn": "Як гэта працуе",
    "how-it-works": "Файлы праграмы захоўваюцца на вашым Google Drive. Праграма будзе мець доступ толькі да створаным ім файлаў",
    "auto-backup": "Выконваць рэзервовае капіраванне аўтаматычна",
    "error-sign-in": "Не атрымалася ўвайсці ў акаўнт",
    "error-sign-out": "Не ўдалося выйсці з акаўнта",
    "error-backup-upload": "Не атрымалася загрузіць рэзервовую копію. Паўторная спроба аўтаматычна будзе выканана пры наступным запуску прыкладання",
    "error-backup-restore": "Не атрымалася восстаносить рэзервовую копію",
    "error-no-backup": "У вас няма рэзервовых копій",

    // common
    "save": "Захаваць",
    "move": "Перайсці",
    "star-app": "Ацаніць праграму",
    "internet-required": "Падключыцеся да інтэрнэту для выканання дзеяння",
    "afternoon": "дзень",
    "evening": "вечар",
    "night": "ноч",
    "morning": "раніца",
    "g-afternoon": "Добры дзень",
    "g-evening": "Добры вечар",
    "g-night": "Дабранач",
    "g-morning": "Добрай раніцы",
    "reload": "Перазапусціць",
    "ok": "Ok",
    "cancel": "Адмяніць",
    "close": "Зачыніць",
    "attention": "Увага",
    "later": "Пазней",
    "app-name": "Штодзённік",
 
    // notes list page
    "no-content": "Няма запісаў",
    "edit": "Рэдагаваць",
    "delete": "Выдаліць",
    "do-copy": "Капіяваць",
    "move-tomorrow": "Перамясціць на заўтра",
    "finished-split": "Завершаныя",
    "reorder-fail": "Не атрымалася перанесьці заметку",

    // add
    "notify": "Нагадаць",
    "set-time-first": "Усталюйце час пачатку",
    "time": "Час",
    "open-galery": "Адкрыць галерэю",
    "make-shot": "Зрабіць здымак",
    "repeat-type": "Паўтараць",
    "repeat-type-no-repeat": "Не паўтараць",
    "repeat-type-day": "Кожны дзень",
    "repeat-type-week": "Кожны тыдзень",
    "repeat-type-any": "Выбраць даты",
    "input-placeholder-title": "Загаловак",
    "input-placeholder-text": "Тэкст",
    "list-item-btn": "Пункт спісу",
    "field-btn": "Поле",
    "update-current": "Абнавіць толькі актуальную",
    "update-all": "Абнавіць усе",
    "update-shadow": "Абнавіць толькі не закранутыя",
    "specify-btn": "Пазначыць",
    "error-note-add": "Не атрымалася дадаць нататку",
    "error-note-update": "Не атрымалася абнавіць нататку",
    "error-note-delete": "Не атрымалася выдаліць заметку",
    "error-get-notes": "Не атрымалася атрымаць нататкі",
    "monday": "Панядзелак",
    "tuesday": "Аўторак",
    "wednesday": "Серада",
    "thursday": "Чацвер",
    "friday": "Пятніца",
    "saturday": "Субота",
    "sunday": "Нядзеля",
    "error-notification-permission": "У вас няма правоў, каб дадаваць апавяшчэння",
    "night-mode": "Начны рэжым",
    "add-content-item": "Дадаць",
    "busy-period-notification": "На гэтую дату ёсць нататка з часам",

    // note-item
    "attached-image": "Прымацаванаы малюнак",

    // error
    "error-repeat-common": "Адбылася памылка. Паўтарыце пазней",
    "error-common": "Адбылася памылка",
    "error-reload-app": "Перазапусціце прыкладанне",

    // trash
    "remove-old": "Нататкі будуць выдаленыя праз 30 дзён",
    "clean-trash": "Ачысціць кошык",
    "clean-trash-confirmation": "Ачысціць кошык?",
    "clean-trash-error": "Не атрымалася ачысціць кошык",
    "error-note-get-trash": "Не атрымалася атрымаць аддаленыя нататкі",
    "error-note-restore": "Не атрымалася аднавіць заметку",
    "deleted-date": "Дата выдалення",

    // notification
    "default-notification-title": "Апавяшчэнне аб нататцы",

    // search
    "search-placeholder": "Пошук",
    "search": "Пошук",
    "no-search-content": "Нічога не знойдзена",

    // menu
    "show-daily-notes-screen": "Панэль Штодзённік",
    "show-notes-screen": "Панэль Нататкі",
    "trash": "Кошык",
    "about": "Аб праграме",
    "settings": "Налады",
    "backups": "Рэзервовае капіраванне",

    // tags
    "tags": "Цэтлікі",
    "edit-tags": "Рэдагаваць цэтлікі",
    "add-tag-placeholder": "Увядзіце назву цэтліку",

    // initial note
    "initial-note-title": "Сардэчна запрашаем! Трохі карыснай інфармацыі для працы з праграмай",
    "initial-note-ci-1": "Націсніце на заметку што-б убачыць поўнае ўтрыманне.",
    "initial-note-ci-2": "Нататка можа ўтрымліваць у сабе:",
    "initial-note-ci-3": "Звычайны тэкст",
    "initial-note-ci-4": "Спіс",
    "initial-note-ci-5": "Пункт 1",
    "initial-note-ci-6": "Пункт 2",
    "initial-note-ci-7": "Пункт 3",
    "initial-note-ci-8": "Малюнак. Для павелічэння малюнка націсніце на яго",
    "initial-note-ci-9": "Таксама для нататкі можна дадаць:",
    "initial-note-ci-10": "Каляровую пазнаку",
    "initial-note-ci-11": "Цэтлікі",
    "initial-note-ci-12": "Напамін",
    "initial-note-ci-13": "Аўтаматычнае паўтарэнне",
    "initial-note-ci-14": "Для перамяшчэння нататкі зацісніце яе на кароткі час і затым перацягнуць на патрэбную пазіцыю",
    "initial-note-ci-15": "У дадатку рэалізавана дзве панэлі.",
    "initial-note-ci-16": "На панэлі \"Штодзённік\" нататкі ствараюцца з прывязкай да даты і адлюстроўваюцца для абранай на календары даты. \n"
        + "Для змены дня націсніце дату на календары або свайпните налева або направа панэль са спісам нататак. Націсніце на стрэлку для адлюстравання разгорнутага календара. \n"
        + "Свайпните каленжарь для паказу іншых дат",
    "initial-note-ci-17": "На панэлі \"Нататкі\" нататкі ствараюцца без прывязкі да даты.",
    "initial-note-ci-18": "Націснуць на даце над календаром для пераходжу на бягучы дзень.",
    "initial-note-ci-19": "Прысутнічае магчымасць дадання фішкі прыкладання на галоўны экран.",
    "initial-note-ci-20": "Прыемнага карыстання!",

    // initial tags
    "initial-tag-1": "Праца",
    "initial-tag-2": "Пакупкі",
    "initial-tag-3": "Ідэі",

    // updates note
    "updates-note-title": "Версія 1.7. Абнаўлення.",
    "updates-note-ci-1": "Дададзеныя цэтлікі.",
    "updates-note-ci-2": "Дададзена перасоўванне нататак ўручную. Для перамяшчэння нататкі зацісніце яе на кароткі час і затым перацягнуць на патрэбную пазіцыю.",
    "updates-note-ci-3": "Дададзеныя панэлі \"Штодзённік\" і \"Нататкі\" \n"
        + "На панэлі \"Штодзённік\" нататкі ствараюцца з прывязкай да даты і адлюстроўваюцца для абранай на календары даты. \n"
        + "На панэлі \"Нататкі\" нататкі ствараюцца без прывязкі да даты.",
    "updates-note-ci-4": "Дададзеная кошык",
    "updates-note-ci-5": "Дададзены пошук нататак",
    "updates-note-ci-6": "Дададзена аўтаматычнае рэзервовае капіраванне",
    "updates-note-ci-7": "Дададзеная магчымасць сбросса пароля",
    "updates-note-ci-8": "Дададзеная магчымасць дадання фішкі прыкладання на галоўны экран",

    // quick add
    "quick-add-placeholder": "Новая нататка...",
}